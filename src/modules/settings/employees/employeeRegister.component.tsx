...
const FormComponent = (props: { close: () => void }) => {
  const { addOne, employees } = useEmployees();
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if ( !name) {
      alert('Falta escorger el nombre');
      return;
    }

    const data: IEmployee = {
      id: employees[employees.length - 1]?.id ? (employees[employees.length - 1].id + 1) : 1,
      name,
    }
